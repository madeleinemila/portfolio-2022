...
const WorkPane: ForwardRefRenderFunction<HTMLDivElement, WorkPaneProps> = ({ jumpToMain }, ref) => {
  const [feature, setFeature] = useState<WorkFeature>(null);

  const handleSCClick = useCallback(() => {
    setFeature("sc");
  }, []);

  const handleAnsaradaClick = useCallback(() => {
    setFeature("ansarada");
  }, []);
    ...
    <div ref={ref} className={styles.container}>
      ...
      <BorderedContent heading="Professional">
        <div className={styles.thumbnailContainer}>
          <Thumbnail imageData={safetyCultureImage} label="Software Engineer @ SafetyCulture" onClick={handleSCClick} />
          <Thumbnail imageData={ansaradaImage} label="Software Engineer @ Ansarada" onClick={handleAnsaradaClick} />
        </div>
      </BorderedContent>
